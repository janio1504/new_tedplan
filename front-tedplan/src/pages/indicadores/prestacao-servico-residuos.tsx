...
export default function PrestacaoServicoResiduos() {
  ...
  async function getMenus() {
    const res = await api.get("menus/eixo/" + 4).then((response) => {
  ...
  async function loadUnidades() {
    ...
    try {
      ...
      const response = await apiClient.get("/unidades/eixo/4");
  ...
  async function loadTiposUnidade(idEixo: number = 4) {
    try {
      const apiClient = getAPIClient();
  ...
  async function handleSaveUnidade(data: any) {
    try {
      ...
      const unidadeData = {
        ...
        id_eixo: parseToIntOrNull(data.id_eixo) || 4,
