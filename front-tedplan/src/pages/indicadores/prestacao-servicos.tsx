...
export default function PrestacaoServicos({ Imunicipio }: MunicipioProps) {
  ...
  const ServiceIcon = styled.div<{ rotation: number }>`
    ...
    align-items: center;
    cursor: pointer;
    transform: rotate(${(props) => props.rotation}deg) translateX(250px)
      ...
      <CircularContainer>
        <CenterIcon>
          <Image
            ...
            alt="Institucional"
          />
          {title === "Institucional" && show && (
            <TitlePsOnMouse>{title}</TitlePsOnMouse>
          )}
        </CenterIcon>

        <div style={{ position: "relative", width: 500, height: 500}}>
          <Image src={Modelo} width={500} height={500} alt="Logo" />
