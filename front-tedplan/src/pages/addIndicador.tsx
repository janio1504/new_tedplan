...
export default function AddIndicador({ indicador, menuItems }: IndicadorProps) {
  ...
  const [allMenuItems, setAllMenuItems] = useState<any>(menuItems);
  ...
  // Filtrar menuItems quando o eixo mudar
  useEffect(() => {
    if (selectedEixo) {
      // Filtrar menuItems cujo menu pertence ao eixo selecionado
      const filtered = allMenuItems.filter((menuItem: IMenuItem) => {
        return menuItem.menu?.id_eixo?.toString() === selectedEixo.toString();
      });
  ...
  async function getMenuItems() {
    try {
      ...
      setAllMenuItems(resMenuItems.data);
  ...
  async function loadIndicadorData(id: string) {
    try {
      ...
      // Se o indicador tiver um menuItem, buscar o menu para pegar o id_eixo
      if (indicador.id_menu_item) {
        try {
          const menuItemResponse = await apiClient.get(
            `/menu-items/${indicador.id_menu_item}`
          );
          const menuItemData = menuItemResponse.data;

          // Se o menuItem tiver um menu com eixo, definir o eixo selecionado primeiro
          if (menuItemData.menu?.id_eixo) {
            setSelectedEixo(menuItemData.menu.id_eixo.toString());
            // Aguardar um pouco para que o filtro de menuItems seja aplicado
            setTimeout(() => {
              reset({
          ...
          console.error("Erro ao carregar menuItem:", error);
          // Continuar mesmo se houver erro ao buscar o menuItem
          reset({
            codigo_indicador: indicador.codigo_indicador,
            nome_indicador: indicador.nome_indicador,
            palavra_chave: indicador.palavra_chave,
      ...
      toast.error("Erro ao carregar dados do indicador!", {
        position: "top-right",
        autoClose: 5000,
      });
    }
  ...
  async function loadTipoCampoIndicador(indicadorId: string) {
    try {
      ...
      const response = await apiClient.get(
        `/tipos-campo/indicador/${indicadorId}`
      );
  ...
  async function loadSelectOptions(tipoCampoId: string) {
    try {
      const apiClient = getAPIClient();
      const response = await apiClient.get(
        `/select-options/tipo-campo/${tipoCampoId}`
      );
  ...
  function addSelectOption() {
    if (!newOption.value.trim() || !newOption.descricao.trim()) {
      toast.warning("Preencha valor e descrição da opção!", {
        position: "top-right",
        autoClose: 5000,
      });
      return;
  ...
  function addCheckBoxItem() {
    if (!newCheckBoxItem.descricao.trim()) {
      toast.warning("Preencha descrição do item!", {
        position: "top-right",
        autoClose: 5000,
      });
      return;
  ...
  async function handleAddIndicador({
    ...
    try {
      ...
      if (isEditing && indicadorId) {
        ...
        indicadorResponse = await apiClient.put(
          `/indicadores-novo/${indicadorId}`,
          indicadorData
        );
        ...
        indicadorResponse = await apiClient.post(
          "/indicadores-novo",
          indicadorData
        );
      ...
      const indicadorIdToUse = isEditing
        ? indicadorId
  ...
  async function saveTipoCampoIndicador(
    indicadorId: string,
    tipoCampoData: any
  ) {
    try {
      ...
      const tipoCampoResponse = await apiClient.post(
        "/tipos-campo",
        tipoCampoCompleto
      );
      ...
      toast.error("Erro ao salvar tipo de campo!", {
        position: "top-right",
        autoClose: 5000,
      });
    }
  ...
  async function loadCheckBoxItems(indicadorId: string) {
    try {
      const apiClient = getAPIClient();
      const response = await apiClient.get(
        `/item-check-box/indicador/${indicadorId}`
      );
          ...
          >
            ...
            >
              ...
              >
                {isEditing
                  ...
                  : "Preencha as informações para criar um novo indicador"}
              </p>
            </div>

            <form
              onSubmit={handleSubmit(handleAddIndicador)}
              style={{
                display: "flex",
                flexDirection: "column",
                gap: "24px",
            ...
            >
              <div style={{ marginBottom: "0" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    padding: "12px 16px",
                    border: `1px solid ${
                      errors.codigo_indicador ? "#e74c3c" : "#e0e0e0"
                    }`,
                    borderRadius: "8px",
                    fontSize: "14px",
                ...
                />
                {errors.codigo_indicador &&
                  errors.codigo_indicador.type === "required" && (
                    <span
                      style={{
                        color: "#e74c3c",
                        fontSize: "12px",
              ...
              <div style={{ marginBottom: "0" }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    padding: "12px 16px",
                    border: `1px solid ${
                      errors.nome_indicador ? "#e74c3c" : "#e0e0e0"
                    }`,
                    borderRadius: "8px",
                    fontSize: "14px",
                    transition: "all 0.3s ease",
                ...
                {errors.nome_indicador &&
                  errors.nome_indicador.type === "required" && (
                    <span
                      style={{
                        color: "#e74c3c",
                        fontSize: "12px",
              ...
              <div style={{ marginBottom: "0" }}>
                ...
                >
                  Selecione um eixo para filtrar os itens de menu disponíveis
                </small>
              </div>
              ...
              >
                ...
                <div style={{ marginBottom: "20px" }}>
                  ...
                  <select
                    ...
                    style={{
                      ...
                      padding: "12px 16px",
                      border: `1px solid ${
                        errors.tipo_campo ? "#e74c3c" : "#e0e0e0"
                      }`,
                      borderRadius: "8px",
                      fontSize: "14px",
                      transition: "all 0.3s ease",
                  ...
                  {errors.tipo_campo &&
                    errors.tipo_campo.type === "required" && (
                      <span
                        style={{
                          color: "#e74c3c",
                          fontSize: "12px",
                    ...
                    {selectOptions.length > 0 && (
                      <div style={{ marginTop: "15px" }}>
                        ...
                        {selectOptions.map((option, index) => (
                          <div
                            key={option.tempId || option.id_select_option}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: "10px",
                ...
                {tipoCampoSelecionado === "checkbox" && (
                  ...
                  >
                    ...
                    >
                      Adicione os itens que aparecerão como opções de checkbox
                      no formulário.
                    </p>
                    ...
                    {checkBoxItems.length > 0 && (
                      <div style={{ marginTop: "15px" }}>
                        ...
                        {checkBoxItems.map((item, index) => (
                          <div
                            key={item.tempId || item.id_item_check_box}
                            style={{
                              display: "flex",
                              alignItems: "center",
                              gap: "10px",
