...
class FinanceiroController {
  ...
  async getFinanceiro({ request }) {
    const { id_municipio, ano } = request.all();
    const financeiro = await Financeiro.query()
      .from("tedplan.financeiro")
      .where("id_municipio", id_municipio)
      .where("ano", ano)
      .fetch();

    financeiro.map((value, key) => {});

    return financeiro.data;
  }
