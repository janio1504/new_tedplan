...
class PsResiduosColetaController {
  ...
  async removerUnidadeRsc({ request }) {
    try {
      ...
      if (res === 0) {
        return { success: false, message: `Sem registros para deletar com ID: ${id}` }
      }
      return { success: true, message: 'Unidade RSC removida com sucesso!', deletedCount: res };
    } catch (error) {
      console.error('Error removing Unidade RSC:', error);
      console.log(error);
    }
  }
  ...
  async removerUnidadeRss({ request }) {
    try {
      ...
      return { success: true, message: 'Unidade RSS removida com sucesso!', deletedCount: res };
    } catch (error) {
      console.error('Error removing Unidade RSS:', error);
      throw error;
    }
  }

  async createUnidadeRsc({ request, response }) {
    ...
    try {
      ...
      return response.status(201).json({
        success: true,
        message: 'Unidade RSC criada com sucesso',
      })
    } catch (error) {
      console.error('Erro ao criar unidade RSC:', error)
      return response.status(400).json({
        success: false,
        message: 'Erro ao criar unidade RSC',
        error: error.message
      })
    }
  }
  ...
  async createUnidadeRss({ request, response }) {
    const { municipio, nome_unidade, operador_unidade,
      cnpj_unidade, quant_residuos_exportados, ano, id_municipio }
      = request.all()
    try {
      ...
      return response.status(201).json({
        success: true,
        message: 'Unidade RSS criada com sucesso',
      })
    } catch (error) {
      console.error('Erro ao criar unidade RSS:', error)
      return response.status(400).json({
        success: false,
        message: 'Erro ao criar unidade RSS',
        error: error.message
      })
    }
  }
  ...
  async createCooperativaCatadores({ request, response }) {
    const dados = request.all()
    try {
      await PsFinanceiro.query()
        .from('tedplan.cooperativa_associacao_catadores')
      ...
      return response.status(201).json({
        success: true,
        message: 'Cooperativa catadores criada com sucesso',
      })
    } catch (error) {
      console.error('Erro ao criar Cooperativa catadores:', error)
      return response.status(400).json({
        success: false,
        message: 'Erro ao criar Cooperativa catadores',
        error: error.message
      })
    }
  }
  ...
  async destroyCAC({ request }) {
    ...
    try {
      ...
      return { success: true, message: 'Cooperativa catadores removida com sucesso!', deletedCount: res };
    } catch (error) {
      console.error('Error removing Cooperativa catadores:', error);
      console.log(error);
    }
  }
