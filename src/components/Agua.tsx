...
export default function Agua({ municipio }: MunicipioProps) {
  ...
  async function IN002(data) {
    ...
    const rsRa = await api.post('get-agua', { id_municipio: data.id_municipio })
      .then(response => { return response.data })

    const rs = await Promise.all(rsRa.map(async (response: any) => {
      const rsGe = await api.post('get-geral-por-ano', { id_municipio: data.id_municipio, ano: response.ano })
        .then(response => { return response.data[0] })

      const rsRe = await api.post('get-esgoto-por-ano', { id_municipio: data.id_municipio, ano: response.ano })
        .then(response => { return response.data[0] })

      const AG003 = response?.ag003
      const ES003 = rsRe?.es003
      const FN026 = rsGe?.fn026
      const result = (parseInt(AG003) + parseInt(ES003)) / parseInt(FN026)
      const ano = response.ano.toString()
  ...
  async function IN003(data) {
    ...
    const rs = await Promise.all(rsRa.map(async (response: any) => {
      ...
      const rsFn = await api.post('get-ps-financeiro-por-ano', { id_municipio: data.id_municipio, ano: response.ano })
        .then(response => { return response.data[0] })
      ...
      const AG011 = response?.ag011
      const ES007 = rsRe?.es007
      const result = (parseFloat(FN017) / (parseFloat(AG011) + parseFloat(ES007))) * (1 * 1000)
      const ano = response.ano.toString()
      const dados = [ano, parseFloat(result.toFixed(2))]      
